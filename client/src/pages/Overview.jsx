    ...
    useEffect(() => {
        const fetchPrices = async () => {
            try {
                ...
                const cryptoFilteredData = data.filter(crypto => allCrypto.includes(crypto.Ticker)).slice(0, 5);
                const stockFilteredData = data.filter(crypto => allStock.includes(crypto.Ticker)).slice(0, 5);
